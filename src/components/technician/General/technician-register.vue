...
export default {
    ...
    methods: {
        register(){
            new usersServices().register(this.email,this.password,this.firstName,this.lastName,this.address,this.phone,"technician").then(response=>console.log("USER CREATED"))
            this.$router.push("/")
        },
        redirectToLogin(){
            this.$router.push("/")
